...
namespace Core::Graphics {
    ...
    template<class T>
    concept CSwapChain = requires(T a) {
        a.addEventListener(declval<ISwapChainEventListener*>());
        a.removeEventListener(declval<ISwapChainEventListener*>());

        { a.setWindowMode(declval<Vector2U>()) } -> convertible_to<bool>;
        { a.setCanvasSize(a.getCanvasSize()) } -> convertible_to<bool>;

        a.clearRenderAttachment();
        a.applyRenderAttachment();
        a.setVSync(false);

        { a.present() } -> convertible_to<bool>;
    };
}
