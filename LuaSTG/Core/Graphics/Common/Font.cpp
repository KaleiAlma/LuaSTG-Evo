...
#define G_FT_Library (SharedFreeTypeLibrary::get().lib)
    ...
    void TrueTypeGlyphManager::onDeviceCreate() {
        // wait for someone to call flush
    }
    void TrueTypeGlyphManager::onDeviceDestroy() {
        // mark all as dirty
        for (auto& t : m_tex) {
            t.dirty_l = 0;
            t.dirty_t = 0;
    ...
    void TrueTypeGlyphManager::closeFonts() {
    ...
    bool TrueTypeGlyphManager::openFonts(TrueTypeFontInfo* fonts, size_t count) {
        closeFonts();
        if (!fonts || count == 0) {
    ...
    bool TrueTypeGlyphManager::addTexture() {
        m_tex.emplace_back();
        auto& t = m_tex.back();
    ...
    bool TrueTypeGlyphManager::findGlyph(FT_ULong code, FT_Face& face, FT_UInt& index) {
        for (auto& f : m_font) {
            if (f.ft_face) {
    ...
    bool TrueTypeGlyphManager::writeBitmapToCache(GlyphCacheInfo& info, FT_Bitmap& bitmap) {
        // too big. get out
    ...
    GlyphCacheInfo* TrueTypeGlyphManager::getGlyphCacheInfo(uint32_t codepoint) {
    ...
    bool TrueTypeGlyphManager::renderCache(uint32_t codepoint) {
    ...
    float TrueTypeGlyphManager::getLineHeight() {
        return m_common_info.ft_line_height;
    }
    float TrueTypeGlyphManager::getAscender() {
        return m_common_info.ft_ascender;
    }
    float TrueTypeGlyphManager::getDescender() {
        return m_common_info.ft_descender;
    }

    uint32_t TrueTypeGlyphManager::getTextureCount() {
        return (uint32_t)m_tex.size();
    }
    ITexture2D* TrueTypeGlyphManager::getTexture(uint32_t index) {
        if (index < m_tex.size()) {
    ...
    bool TrueTypeGlyphManager::cacheGlyph(uint32_t codepoint) {
        if (!getGlyphCacheInfo(codepoint))
            return false;
        return true;
    }
    bool TrueTypeGlyphManager::cacheString(StringView str) {
        // utf-8 迭代器
        char32_t code_ = 0;
    ...
    bool TrueTypeGlyphManager::flush() {
        for (auto& t : m_tex) {
            if (t.dirty_l != INVALID_RECT) {
    ...
    bool TrueTypeGlyphManager::getGlyph(uint32_t codepoint, GlyphInfo* p_ref_info, bool no_render) {
        if (!p_ref_info) {
            assert(false); return false;
        }
    ...
    TrueTypeGlyphManager::TrueTypeGlyphManager(IDevice* p_device, TrueTypeFontInfo* p_arr_info, size_t info_count)
        : m_device(p_device) {
        if (!openFonts(p_arr_info, info_count)) {
            throw std::runtime_error("TrueTypeGlyphManager::TrueTypeGlyphManager (openFonts)");
        }
        if (!addTexture()) {
            throw std::runtime_error("TrueTypeGlyphManager::TrueTypeGlyphManager (addTexture)");
        }
        cacheGlyph(uint32_t(U' ')); // 先缓存一个空格
        m_device->addEventListener(this);
    }
    TrueTypeGlyphManager::~TrueTypeGlyphManager() {
    ...
    bool IGlyphManager::create(IDevice* p_device, TrueTypeFontInfo* p_arr_info, size_t info_count, IGlyphManager** pp_glyphmgr) {
        try {
            *pp_glyphmgr = new TrueTypeGlyphManager(p_device, p_arr_info, info_count);
            return true;
        }
        catch (...) {
            *pp_glyphmgr = nullptr;
...
namespace Core::Graphics {
    ...
    bool TextRenderer::drawText(StringView str, Vector2F const& start, Vector2F* endout) {
        ...
        // Position parameters
        GlyphInfo glyph_info = {};
        Vector2F start_pos = start; // Stroke position
        float const line_height = m_glyphmgr->getLineHeight() * m_scale.y; // Line height
    ...
    bool ITextRenderer::create(IRenderer* p_renderer, ITextRenderer** pp_textrenderer) {
        try {
            *pp_textrenderer = new TextRenderer(p_renderer);
            return true;
        }
        catch (...) {
