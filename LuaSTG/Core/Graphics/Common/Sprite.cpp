...
namespace Core::Graphics {
	...
	void Sprite::setUnitsPerPixel(float v) {
		m_scale = v;
		updateRect();
	}
	...
	void Sprite::draw(Vector3F const& pos, Vector3F const& rot, Vector2F const& scale) {
		m_renderer->setTexture(m_texture.get());
...
#undef transform_xyz
	...
	void Sprite::draw(Vector2F const& pos, float scale) {
		draw(pos, Vector2F(scale, scale));
	}
	void Sprite::draw(Vector2F const& pos, float scale, float rotation) {
	...
	void Sprite::draw(Vector2F const& pos, Vector2F const& scale, float rotation) {
		if (std::abs(rotation) < std::numeric_limits<float>::min()) {
			draw(pos, scale);
			return;
		}
...
#undef rotate_xy
	...
	bool Sprite::clone(ISprite** pp_sprite) {
		try {
			*pp_sprite = new Sprite(m_renderer.get(), m_texture.get());
			Sprite* right = static_cast<Sprite*>(*pp_sprite);
			right->m_rect = m_rect;
	...
	Sprite::Sprite(IRenderer* p_renderer, ITexture2D* p_texture)
