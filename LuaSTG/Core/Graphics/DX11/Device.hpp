...
namespace Core::Graphics::DX11
{
	...
	public:
		Device(std::string_view const& prefered_gpu = "");
		~Device();

	public:
		static bool create(StringView prefered_gpu, Device** p_device);
	};
	...
	public:
		SamplerState(Device* p_device, SamplerState const& def);
		~SamplerState();
	};
	...
	public:
		...
		Texture2D(Device* device, Vector2U size, bool rendertarget); // rendertarget = true 时不注册监听器，交给 RenderTarget 控制
		~Texture2D();
	};
