...
namespace Core::Graphics::DX11
{
    ...
    bool ModelSharedComponent::createResources()
    {
    ...
    void ModelSharedComponent::onDeviceCreate()
    {
        createResources();
    }
    void ModelSharedComponent::onDeviceDestroy()
    {
        default_image.Reset();
    ...
    ModelSharedComponent::ModelSharedComponent(Device* p_device)
        : m_device(p_device)
    {
        if (!createResources())
            throw std::runtime_error("ModelSharedComponent::ModelSharedComponent");
...
namespace Core::Graphics::DX11
{
    void map_sampler_to(tinygltf::Sampler& samp, D3D11_SAMPLER_DESC& desc)
    {
    ...
    void map_primitive_topology_to(tinygltf::Primitive& prim, D3D11_PRIMITIVE_TOPOLOGY& topo)
    {
        switch (prim.mode)
        {
        default:
    ...
    bool Model::processNode(tinygltf::Model& model, tinygltf::Node& node)
    {
        ...
        if (node.mesh >= 0)
        {
            ...
            for (tinygltf::Primitive& prim : mesh.primitives)
            {
                ...
                map_primitive_topology_to(prim, mblock.primitive_topology);
    ...
    void Model::onDeviceCreate()
    {
        createResources();
    }
    void Model::onDeviceDestroy()
    {
        image.clear();
        sampler.clear();
    ...
    void Model::draw(IRenderer::FogState fog)
    {
        auto* context = m_device->GetD3D11DeviceContext();
    ...
    Model::Model(Device* p_device, ModelSharedComponent* p_model_shared, StringView path)
        : m_device(p_device)
        ...
        , gltf_path(path)
    {
        ...
        if (!createResources())
            throw std::runtime_error("Model::Model");
        m_device->addEventListener(this);
    }
    Model::~Model()
    {
