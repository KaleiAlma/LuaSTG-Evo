...
namespace Core::Graphics::DX11
{
	inline ID3D11ShaderResourceView* get_view(Texture2D* p)
	{
	...
	inline ID3D11ShaderResourceView* get_view(ScopeObject<Texture2D>& p)
	{
...
namespace Core::Graphics::DX11
{
	...
	void Renderer::setSamplerState(SamplerState state, UINT index)
	{
		ID3D11SamplerState* d3d11_sampler = static_cast<SamplerState*>(_sampler_state[IDX(state)].get())->GetState();
	...
	void Renderer::bindTextureSamplerState(ITexture2D* texture)
	{
		...
		ID3D11SamplerState* d3d11_sampler = static_cast<SamplerState*>(sampler)->GetState();
	...
	void Renderer::setRenderAttachment(IRenderTarget* p_rt, IDepthStencilBuffer* p_ds)
	{
		batchFlush();
		auto* ctx = m_device->GetD3D11DeviceContext();
		assert(ctx);
		ID3D11RenderTargetView* rtv[1] = { p_rt ? static_cast<RenderTarget*>(p_rt)->GetView() : NULL };
		ID3D11DepthStencilView* dsv = p_ds ? static_cast<DepthStencilBuffer*>(p_ds)->GetView() : NULL;
	...
	bool Renderer::drawTriangle(DrawVertex const* pvert)
	{
		return drawTriangle(pvert[0], pvert[1], pvert[2]);
	}
	bool Renderer::drawQuad(DrawVertex const& v1, DrawVertex const& v2, DrawVertex const& v3, DrawVertex const& v4)
	{
	...
	bool Renderer::drawQuad(DrawVertex const* pvert)
	{
		return drawQuad(pvert[0], pvert[1], pvert[2], pvert[3]);
	}
	bool Renderer::drawRaw(DrawVertex const* pvert, uint16_t nvert, DrawIndex const* pidx, uint16_t nidx)
	{
	...
	bool Renderer::drawRequest(uint16_t nvert, uint16_t nidx, DrawVertex** ppvert, DrawIndex** ppidx, uint16_t* idxoffset)
	{
		if (nvert > _draw_list.vertex.capacity || nidx > _draw_list.index.capacity)
		{
			assert(false); return false;
		...
		ITexture2D* const* p_tex_arr, SamplerState const* sv, size_t tv_sv_n)
	{
		...
		Microsoft::WRL::ComPtr<ID3D11RenderTargetView> p_rtv;
		...
		ctx->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
		ID3D11Buffer* p_vbos[1] = { _fx_vbuffer.Get() };
		UINT const stride = sizeof(DrawVertex);
		UINT const offset = 0;
		ctx->IASetVertexBuffers(0, 1, p_vbos, &stride, &offset);
		...
		ctx->PSSetShader(static_cast<PostEffectShader*>(p_effect)->GetPS(), NULL, 0);
		...
		ID3D11RenderTargetView* p_rtvs[1] = { p_rtv.Get() };
		ctx->OMSetRenderTargets(1, p_rtvs, p_dsv.Get());
	...
	ISamplerState* Renderer::getKnownSamplerState(SamplerState state)
	{
	...
	bool IRenderer::create(IDevice* p_device, IRenderer** pp_renderer)
	{
		try
		{
			*pp_renderer = new Renderer(static_cast<Device*>(p_device));
			return true;
		}
		catch (...)
		{
