...
namespace Core::Graphics::DX11
{
	...
	Device::Device(std::string_view const& prefered_gpu)
		: preferred_adapter_name(prefered_gpu)
	{
		// 创建图形组件

		i18n_log_info("[core].Device.start_creating_graphic_components");
		...
		i18n_log_info("[core].Device.created_graphic_components");
	}
	Device::~Device()
	{
		// 清理对象
	...
	bool Device::selectAdapter()
	{
		...
		i18n_log_info("[core].Device.enum_all_adapters");
		...
		for (UINT idx = 0; bHR = dxgi_factory->EnumAdapters1(idx, &dxgi_adapter_temp); idx += 1)
		{
			...
			if (bHR = gHR = dxgi_adapter_temp->GetDesc1(&desc_))
			{
				...
				i18n_log_info_fmt("[core].Device.DXGI_adapter_detail_fmt"
					, idx
					, dev_name
				...
				if (soft_dev_type)
				{
					supported = false; // 排除软件或远程设备
				}
			}
			else
			{
				i18n_core_system_call_report_error("IDXGIAdapter1::GetDesc1");
				i18n_log_error_fmt("[core].Device.DXGI_adapter_detail_error_fmt", idx);
				supported = false; // 排除未知错误
			}
			...
			for (UINT odx = 0; bHR = dxgi_adapter_temp->EnumOutputs(odx, &dxgi_output_temp); odx += 1)
			{
				...
				if (read_o_desc)
				{
					i18n_log_info_fmt("[core].Device.DXGI_output_detail_fmt"
						, idx, odx
				...
				{
					i18n_log_error_fmt("[core].Device.DXGI_output_detail_error_fmt", idx, odx);
				}
			}
		...
		if (dxgi_adapter)
		{
			M_D3D_SET_DEBUG_NAME(dxgi_adapter.Get(), "Device::dxgi_adapter");
			i18n_log_info_fmt("[core].Device.select_DXGI_adapter_fmt", dxgi_adapter_name);
			if (!link_to_output)
			{
				i18n_log_warn_fmt("[core].Device.DXGI_adapter_no_output_warning_fmt", dxgi_adapter_name);
			}
		}
		else
		{
			i18n_log_critical("[core].Device.no_available_DXGI_adapter");
			return false;
		}
	...
	bool Device::createDXGI()
	{
		...
		i18n_log_info("[core].Device.start_creating_basic_DXGI_components");
		...
		// 打印特性支持情况
		i18n_log_info_fmt("[core].Device.DXGI_detail_fmt"
		...
		if (dxgi_adapter)
		{
			...
			i18n_log_info("[core].Device.created_basic_DXGI_components");
		}
	...
	bool Device::createD3D11()
	{
		...
		i18n_log_info("[core].Device.start_creating_basic_components");
		...
		M_D3D_SET_DEBUG_NAME(d3d11_device.Get(), "Device::d3d11_device");
		M_D3D_SET_DEBUG_NAME(d3d11_devctx.Get(), "Device::d3d11_devctx");
		if (!dxgi_adapter)
		{
	...
	#undef _CHECK_FORMAT_SUPPORT
		...
		D3D11_FEATURE_DATA_OPTIONS2 d3d11_feature_o2 = {};
		HRESULT hr_o2 = d3d11_device->CheckFeatureSupport(D3D11_FEATURE_OPTIONS2, &d3d11_feature_o2, sizeof(d3d11_feature_o2));
		if (FAILED(hr_o2))
		{
			i18n_core_system_call_report_error("ID3D11Device::CheckFeatureSupport -> D3D11_FEATURE_OPTIONS2");
			// 不是严重错误
		}
	...
	#undef _FORMAT_INFO_STRING_FMT3
		...
		i18n_log_info("[core].Device.created_basic_components");
	...
	bool Device::testMultiPlaneOverlay()
	{
		...
		for (UINT i = 0; bHR = dxgi_factory->EnumAdapters1(i, &adapter_); i += 1)
		{
			...
			for (UINT j = 0; bHR = adapter_->EnumOutputs(j, &output_); j += 1)
			{
				...
				i18n_log_info_fmt("[core].Device.DXGI_output_detail_fmt2"
					, i, j
	...
	bool Device::createTextureFromFile(StringView path, bool mipmap, ITexture2D** pp_texutre)
	{
		try
		{
			*pp_texutre = new Texture2D(this, path, mipmap);
			return true;
		}
		catch (...)
		{
	...
	bool Device::createTexture(Vector2U size, ITexture2D** pp_texutre)
	{
		try
		{
			*pp_texutre = new Texture2D(this, size, false);
			return true;
		}
		catch (...)
		{
	...
	bool Device::createRenderTarget(Vector2U size, IRenderTarget** pp_rt)
	{
		try
		{
			*pp_rt = new RenderTarget(this, size);
			return true;
		}
		catch (...)
		{
	...
	bool Device::createDepthStencilBuffer(Vector2U size, IDepthStencilBuffer** pp_ds)
	{
		try
		{
			*pp_ds = new DepthStencilBuffer(this, size);
			return true;
		}
		catch (...)
		{
	...
	bool Device::createSamplerState(SamplerState const& def, ISamplerState** pp_sampler)
	{
		try
		{
			*pp_sampler = new SamplerState(this, def);
			return true;
		}
		catch (...)
		{
	...
	bool Device::create(StringView prefered_gpu, Device** p_device)
	{
		try
		{
			*p_device = new Device(prefered_gpu);
			return true;
		}
		catch (...)
		{
...
namespace Core::Graphics::DX11
{
	...
	#undef makeColor
		...
		M_D3D_SET_DEBUG_NAME(d3d11_sampler.Get(), "SamplerState::d3d11_sampler");

		return true;
	}

	SamplerState::SamplerState(Device* p_device, SamplerState const& def)
		: m_device(p_device)
		, m_info(def)
	{
		if (!createResource())
			throw std::runtime_error("SamplerState::SamplerState");
	...
	bool Texture2D::uploadPixelData(RectU rc, void const* data, uint32_t pitch)
	{
		if (!m_dynamic)
		{
			return false;
	...
	bool Texture2D::createResource()
	{
		...
		if (m_data)
		{
			...
			M_D3D_SET_DEBUG_NAME(d3d11_texture2d.Get(), "Texture2D::d3d11_texture2d");
			...
			M_D3D_SET_DEBUG_NAME(d3d11_srv.Get(), "Texture2D::d3d11_srv");
		}
	...
	Texture2D::Texture2D(Device* device, StringView path, bool mipmap)
		: m_device(device)
		, source_path(path)
		, m_dynamic(false)
		, m_premul(false)
	...
	Texture2D::Texture2D(Device* device, Vector2U size, bool rendertarget)
		: m_device(device)
		, m_size(size)
		, m_dynamic(true)
	...
	bool RenderTarget::setSize(Vector2U size)
	{
		d3d11_rtv.Reset();
	...
	bool RenderTarget::createResource()
	{
		...
		M_D3D_SET_DEBUG_NAME(d3d11_rtv.Get(), "RenderTarget::d3d11_rtv");
	...
	RenderTarget::RenderTarget(Device* device, Vector2U size)
		: m_device(device)
	{
		m_texture.attach(new Texture2D(device, size, true));
		if (!createResource())
	...
	bool DepthStencilBuffer::setSize(Vector2U size)
	{
		d3d11_texture2d.Reset();
		d3d11_dsv.Reset();
		m_size = size;
	...
	bool DepthStencilBuffer::createResource()
	{
		...
		M_D3D_SET_DEBUG_NAME(d3d11_texture2d.Get(), "DepthStencilBuffer::d3d11_texture2d");
		...
		M_D3D_SET_DEBUG_NAME(d3d11_dsv.Get(), "DepthStencilBuffer::d3d11_dsv");

		return true;
	}

	DepthStencilBuffer::DepthStencilBuffer(Device* device, Vector2U size)
		: m_device(device)
		, m_size(size)
	{
		if (!createResource())
