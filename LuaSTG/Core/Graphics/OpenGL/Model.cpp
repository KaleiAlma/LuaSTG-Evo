...
namespace Core::Graphics::OpenGL {
    void map_sampler_to(tinygltf::Sampler& samp, GLuint tex) {
    ...
    void map_primitive_topology_to(tinygltf::Primitive& prim, GLenum& topo) {
        switch (prim.mode) {
        default:
            topo = GL_TRIANGLES;
            break;
            ...
            for (tinygltf::Primitive& prim : mesh.primitives) {
                ...
                #pragma warning(default:4244)
                    ...
                    if (texture_info.index >= 0) {
                        ...
                        map_sampler_to(mblock.sampler, mblock.image);
                        glGenerateMipmap(GL_TEXTURE_2D);
                    } else {
