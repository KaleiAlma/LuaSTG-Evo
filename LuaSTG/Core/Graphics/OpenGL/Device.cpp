...
namespace Core::Graphics::OpenGL {
    ...
    bool Device::createTextureFromFile(StringView path, bool mipmap, ITexture2D** pp_texture) {
        try {
            *pp_texture = new Texture2D(this, path, mipmap);
            return true;
        }
        catch (...) {
            *pp_texture = nullptr;
    ...
    bool Device::createTextureFromMemory(void const* data, size_t size, bool mipmap, ITexture2D** pp_texture) {
        try {
            *pp_texture = new Texture2D(this, data, size, mipmap);
            return true;
        }
        catch (...) {
            *pp_texture = nullptr;
    ...
    bool Device::createTexture(Vector2U size, ITexture2D** pp_texture) {
        try {
            *pp_texture = new Texture2D(this, size, false);
            return true;
        }
        catch (...) {
            *pp_texture = nullptr;
    ...
    bool Device::create(Device** p_device) {
        try {
            *p_device = new Device();
            return true;
        }
...
namespace Core::Graphics::OpenGL {
    ...
    Texture2D::Texture2D(Device* device, void const* data, size_t size, bool mipmap)
        : m_device(device)
        , m_dynamic(false)
        , m_premul(false)
        , m_mipmap(mipmap)
    ...
    bool RenderTarget::createResource() {
        ...
        glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_texture->GetResource(), 0);
    ...
    RenderTarget::RenderTarget(Device* device, Vector2U size)
        ...
        m_depthstencilbuffer.attach(new DepthStencilBuffer(device, size));
        if (!createResource())
