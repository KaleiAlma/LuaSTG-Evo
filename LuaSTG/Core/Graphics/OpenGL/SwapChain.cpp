...
namespace Core::Graphics::OpenGL {
    ...
    void SwapChain::onWindowCreate() {
        // Currently window recreations are only triggered by the swapchain, so leave this blank
    }
    ...
    void SwapChain::onWindowSize(Core::Vector2I size) {
    ...
    bool SwapChain::handleSwapChainWindowSize(Vector2I size) {
    ...
    bool SwapChain::present() {
        ...
        Vector2I wsize{};
        SDL_GetWindowSizeInPixels(m_window->GetWindow(), &wsize.x, &wsize.y);
    ...
    bool SwapChain::addFramebuffer(GLuint &fbo, GLuint &tex) {
        glGenTextures(1, &tex);
        if (tex == 0) {
    ...
    SwapChain::SwapChain(Window_SDL* p_window, Device* p_device)
        : m_window(p_window)
        , m_device(p_device) {
        assert(p_window);
        assert(p_device);
    ...
    bool SwapChain::create(Window_SDL* p_window, Device* p_device, SwapChain** pp_swapchain) {
        try {
...
namespace Core::Graphics {
    bool ISwapChain::create(IWindow* p_window, IDevice* p_device, ISwapChain** pp_swapchain) {
        try {
            *pp_swapchain = new OpenGL::SwapChain(dynamic_cast<Window_SDL*>(p_window), dynamic_cast<OpenGL::Device*>(p_device));
            return true;
        }
        catch (...) {
            *pp_swapchain = nullptr;
