...
#include "Core/Graphics/OpenGL/Model.hpp"
...
namespace Core::Graphics::OpenGL {
    inline GLuint get_view(Texture2D* p) {
        return p->GetView();
    }
    inline GLuint get_view(ITexture2D* p) {
        return get_view(static_cast<Texture2D*>(p));
    }
    inline GLuint get_view(ScopeObject<Texture2D>& p) {
        return get_view(p.get());
    }
    inline GLuint get_view(ScopeObject<ITexture2D>& p) {
        return get_view(static_cast<Texture2D*>(p.get()));
    }

    // inline GLuint get_sampler(ISamplerState* p_sampler)
    // {
    // 	return static_cast<SamplerState*>(p_sampler)->GetState();
    // }
    // inline GLuint get_sampler(ScopeObject<ISamplerState>& p_sampler)
    // {
    // 	return static_cast<SamplerState*>(p_sampler.get())->GetState();
    // }
}

namespace Core::Graphics::OpenGL {
    void PostEffectShader::onDeviceCreate() {
        createResources();
    }
    void PostEffectShader::onDeviceDestroy() {
    ...
    bool PostEffectShader::findVariable(StringView name, LocalConstantBuffer*& buf, LocalVariable*& val) {
    ...
    bool PostEffectShader::setTexture2D(StringView name, ITexture2D* p_texture) {
        std::string name_s(name);
        auto it = m_texture2d_map.find(name_s);
        if (it == m_texture2d_map.end()) { return false; }
        it->second.texture = dynamic_cast<Texture2D*>(p_texture);
        if (!it->second.texture) { assert(false); return false; }
        return true;
    }
    bool PostEffectShader::apply(IRenderer* p_renderer) {
        assert(p_renderer);
        ...
        for (auto& v : m_texture2d_map) {
            ...
            static_cast<Renderer*>(p_renderer)->setSamplerState(p_custom.value_or(p_sampler), v.second.index);
        }

        return true;
    }

    void PostEffectShader::bind(GLuint engine_data, GLuint user_data) {
    ...
    PostEffectShader::PostEffectShader(Device* p_device, StringView path, bool is_path_)
        : m_device(p_device)
        , source(path)
        , is_path(is_path_) {
        if (!createResources())
            throw std::runtime_error("PostEffectShader::PostEffectShader");
...
namespace Core::Graphics::OpenGL {
    void Renderer::setVertexIndexBuffer(size_t index) {
    ...
    bool Renderer::uploadVertexIndexBuffer(bool discard) {
        // ZoneScoped;
    ...
    void Renderer::setSamplerState(IRenderer::SamplerState state, GLuint index) {
        // GLuint opengl_sampler = static_cast<SamplerState*>(_sampler_state[IDX(state)].get())->GetState();
    ...
    void Renderer::setSamplerState(Graphics::SamplerState state, GLuint index) {
    ...
    bool Renderer::uploadVertexIndexBufferFromDrawList() {
        // upload data
    ...
    void Renderer::bindTextureAlphaType(ITexture2D* texture) {
    ...
    bool Renderer::batchFlush(bool discard) {
        // ZoneScoped;
        if (!discard) {
    ...
    void Renderer::setRenderAttachment(IRenderTarget* p_rt) {
        batchFlush();
        glBindFramebuffer(GL_DRAW_FRAMEBUFFER, static_cast<RenderTarget*>(p_rt)->GetFramebuffer());
    }
    ...
    inline bool is_same(Texture2D* a, ITexture2D* b) {
        if (a && b)
            return a->GetView() == static_cast<Texture2D*>(b)->GetView();
        else if (!a && !b)
            return true;
        else
            return false;
    }
    inline bool is_same(ScopeObject<Texture2D>& a, ITexture2D* b) {
        return is_same(*a, b);
    }
    ...
    void Renderer::setTexture(ITexture2D* texture) {
        if (_draw_list.command.size > 0 && is_same(_draw_list.command.data[_draw_list.command.size - 1].texture, texture)) {
            ...
            cmd_.texture = static_cast<Texture2D*>(texture);
            cmd_.vertex_count = 0;
            cmd_.index_count = 0;
        }
    ...
    bool Renderer::drawTriangle(DrawVertex const& v1, DrawVertex const& v2, DrawVertex const& v3) {
    ...
    bool Renderer::drawTriangle(IRenderer::DrawVertex const* pvert) {
        return drawTriangle(pvert[0], pvert[1], pvert[2]);
    }
    bool Renderer::drawQuad(IRenderer::DrawVertex const& v1, IRenderer::DrawVertex const& v2, IRenderer::DrawVertex const& v3, IRenderer::DrawVertex const& v4) {
    ...
    bool Renderer::drawQuad(IRenderer::DrawVertex const* pvert) {
        return drawQuad(pvert[0], pvert[1], pvert[2], pvert[3]);
    }
    bool Renderer::drawRaw(IRenderer::DrawVertex const* pvert, uint16_t nvert, DrawIndex const* pidx, uint16_t nidx) {
    ...
    bool Renderer::drawRequest(uint16_t nvert, uint16_t nidx, IRenderer::DrawVertex** ppvert, DrawIndex** ppidx, uint16_t* idxoffset) {
        if (nvert > _draw_list.vertex.capacity || nidx > _draw_list.index.capacity) {
            assert(false); return false;
        }
    ...
    bool Renderer::createPostEffectShader(StringView path, IPostEffectShader** pp_effect) {
        try {
            *pp_effect = new PostEffectShader(m_device.get(), path, true);
            return true;
        }
        catch (...) {
            *pp_effect = nullptr;
    ...
    bool Renderer::drawPostEffect(
        ...
        glUseProgram(static_cast<PostEffectShader*>(p_effect)->GetShader());
        ...
        static_cast<PostEffectShader*>(p_effect)->bind(_fog_data_buffer, _user_float_buffer);

        for (int stage = 0; stage < std::min<int>((int)tv_sv_n, 4); stage++) {
            glActiveTexture(GL_TEXTURE1 + stage);
            glBindTexture(GL_TEXTURE_2D, static_cast<Texture2D*>(p_tex_arr[stage])->GetResource());
            setSamplerState(sv[stage], stage);
        }
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, static_cast<Texture2D*>(p_tex)->GetResource());
    ...
    bool Renderer::drawPostEffect(IPostEffectShader* p_effect, BlendState blend) {
        assert(p_effect);
        ...
        static_cast<PostEffectShader*>(p_effect)->bind(_fog_data_buffer, 0);
    ...
    bool Renderer::createModel(StringView path, IModel** pp_model) {
        if (!m_model_shared) {
            spdlog::info("[core] Creating ModelSharedComponent");
            try {
                *(~m_model_shared) = new ModelSharedComponent(m_device.get());
        ...
        try {
            *pp_model = new Model(m_device.get(), m_model_shared.get(), path);
            return true;
        }
    ...
    bool Renderer::drawModel(IModel* p_model) {
        if (!p_model) {
            assert(false);
            return false;
        }
        ...
        static_cast<Model*>(p_model)->draw(_state_set.fog_state);
    ...
    Graphics::SamplerState Renderer::getKnownSamplerState(SamplerState state) {
        return _sampler_state[IDX(state)];
    }

    Renderer::Renderer(Device* p_device)
        : m_device(p_device) {
        if (!createResources())
    ...
    bool Renderer::create(Device* p_device, Renderer** pp_renderer) {
        try {
            *pp_renderer = new Renderer(p_device);
            return true;
        }
        catch (...) {
            *pp_renderer = nullptr;
...
namespace Core::Graphics {
    bool IRenderer::create(IDevice* p_device, IRenderer** pp_renderer)
    {
        try
        {
            *pp_renderer = new OpenGL::Renderer(static_cast<OpenGL::Device*>(p_device));
            return true;
        }
        catch (...)
        {
