...
#define APPMODEL ((ApplicationModel_SDL*)m_framework)
    void Window_SDL::handleEvents() {
        ...
        while (SDL_PollEvent(&ev)) {
            ...
            case SDL_EVENT_TEXT_INPUT:
                if (!(SDL_GetModState() & (SDL_KMOD_CTRL | SDL_KMOD_LALT))) {
            ...
            case SDL_EVENT_KEY_DOWN:
                switch (ev.key.key) {
                    case SDLK_BACKSPACE:
                        if (SDL_TextInputActive(sdl_window) && m_text_cursor_pos > 0 && !(SDL_GetModState() & SDL_KMOD_ALT)) {
                            // m_text_input = m_text_input
                        ...
                        break;
                    case SDLK_DELETE:
                        if (SDL_TextInputActive(sdl_window) && m_text_cursor_pos < getTextInputLength() && !(SDL_GetModState() & SDL_KMOD_ALT)) {
                            if (SDL_GetModState() & SDL_KMOD_CTRL) {
                        ...
                        break;
                    case SDLK_RETURN:
                        if (SDL_TextInputActive(sdl_window) && m_return_enable && !(SDL_GetModState() & (SDL_KMOD_ALT | SDL_KMOD_CTRL))) {
                            auto view = una::views::grapheme::utf8(m_text_input);
                            auto pos = std::next(view.begin(), m_text_cursor_pos).begin() - m_text_input.begin();
    ...
    bool Window_SDL::createWindow() {
        ...
        m_monitor_idx = SDL_GetDisplayForWindow(sdl_window);
    ...
    bool Window_SDL::recreateWindow() {
        ...
        SDL_DestroyWindow(sdl_window);
        sdl_window = SDL_CreateWindow(sdl_window_text.c_str(), sdl_window_width, sdl_window_height, flags);
    ...
    void Window_SDL::_setWindowMode(Vector2U size, bool ignore_size) {
        ...
        if (want_restore_placement) {
            ...
            int32_t monitor = m_monitor_idx == -1 ? SDL_GetDisplayForWindow(sdl_window) : m_monitor_idx;
    ...
    void Window_SDL::_setFullScreenMode() {
        ...
        SDL_SetWindowPosition(sdl_window, SDL_WINDOWPOS_CENTERED_DISPLAY(monitor), SDL_WINDOWPOS_CENTERED_DISPLAY(monitor));
        SDL_SetWindowFullscreenMode(sdl_window, *SDL_GetFullscreenDisplayModes(SDL_GetDisplayForWindow(sdl_window), NULL));
        SDL_SetWindowFullscreen(sdl_window, true);
    ...
    bool Window_SDL::setFrameStyle(WindowFrameStyle style) {
        ...
        case WindowFrameStyle::None:
            ...
            SDL_SetWindowResizable(sdl_window, false);
            break;
    ...
    bool Window_SDL::setLayer(WindowLayer layer) {
        ...
        case WindowLayer::Normal:
            ...
            SDL_SetWindowAlwaysOnTop(sdl_window, false);
            break;
        case WindowLayer::Top:
            SDL_ShowWindow(sdl_window);
            SDL_SetWindowAlwaysOnTop(sdl_window, true);
            break;
        }
    ...
    bool Window_SDL::setCursor(WindowCursor type) {
        ...
        case WindowCursor::NotAllowed:
            SDL_ShowCursor();
            SDL_SetCursor(SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_NOT_ALLOWED));
            break;
    ...
    void Window_SDL::setTextInputRect(RectI rect) {
        ...
        SDL_SetTextInputArea(sdl_window, &rc, 0);
    }
