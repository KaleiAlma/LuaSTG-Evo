...
namespace Core {
    ...
    ApplicationModel_SDL::ApplicationModel_SDL(IApplicationEventListener* p_listener)
        ...
        if (!Graphics::IDevice::create(~m_device))
            throw std::runtime_error("Graphics::IDevice::create");
        ...
        if (!Graphics::ISwapChain::create(*m_window, *m_device, ~m_swapchain))
            throw std::runtime_error("Graphics::ISwapChain::create");
        if (!Graphics::IRenderer::create(*m_device, ~m_renderer))
            throw std::runtime_error("Graphics::IRenderer::create");
